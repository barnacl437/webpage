...
## About stuff 
These webpages are made lightweight despite having many texts inside and (somewhat) fancy CSS looking. It can be healthy for weak connection, old browsers or crap memory amount, as long as it's hosted on concurrently accessible servers, such as no-SSL/HTTPS or HTTP-only connection. Since this website is hosted on modern infras (vercel) and uses a domain name whose *TLD* (.dev) has a policy for permitting only HTTPS connections (see [HSTS](https://en.wikipedia.org/wiki/HTTP_Strict_Transport_Security) and [HSTS preloading](https://hstspreload.org/)), this website can only be accessible via a modern browser on a modern system with proper and unexpired certificates.
...
Edit3: yeah you know where my new blog is. 
